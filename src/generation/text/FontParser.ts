...
export class FontParser {
        ...
        case 'M': // Move to (start new contour)
          if (currentContour.length > 0) {
            contours.push({
              ...
              isHole: !this.isClockwise(currentContour) // After Y-flip: CW = outer, CCW = hole
            });
            currentContour = [];
          }
          currentContour.push({
  ...
  private extractPathContours(
    ...
    const pushContour = () => {
      ...
      const isHole = polygon.length > 2 ? !this.isClockwise(polygon) : false; // After Y-flip: CW = outer, CCW = hole
   ...
   *
   * In TrueType fonts, outer contours are CCW and holes are CW in Y-up coordinates.
   * Since we flip Y → Z with negation (z = -y), the winding order reverses:
   * - Outer contours become CW (sum < 0)
   * - Holes become CCW (sum > 0)
   *
   * Therefore, a hole is when isClockwise() returns FALSE (CCW after flip).
   */
  private isClockwise(points: Point2D[]): boolean {
    ...
    return sum < 0; // Negative sum = clockwise in XZ plane
  }
